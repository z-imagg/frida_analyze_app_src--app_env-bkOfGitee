...
[[ $(docker images -q  --filter "reference=base_ubuntu_22.04.04" | wc -l ) -gt 0 ]] ||  docker build --progress=plain --no-cache  -f "/fridaAnlzAp/app_bld/env/base_ubuntu_22.04.Dockerfile" -t base_ubuntu_22.04.04:0.1 "/" 
...
docker build --progress=plain --add-host=giteaz:10.0.4.9    --no-cache  -f "/fridaAnlzAp/app_bld/env/fridaAnlzAp.Dockerfile" -t frida_anlz_ap:0.1_prv "/" 

#启动 frida_anlz_ap镜像
# 开发时用的一些选项 ：   --rm  、  -v hostDir:dirInDocker  、  -v  /tmp/app/:/app/ 、  -v  /tmp/fridaAnlzAp:/fridaAnlzAp
# docker ps  -q  --filter "label=frida_anlz_ap"
docker run --privileged=true  --name frida_anlz_ap --hostname faa_dk -itd frida_anlz_ap:0.1_prv
